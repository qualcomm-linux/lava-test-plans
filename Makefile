...
# The audio clip URL is generated by shelling out to "aws s3 presign". Without
# credentials the AWS CLI first probes the EC2 metadata endpoint, which stalls
# for several seconds on a machine that has no instance profile. 1379 of the
# tests render with variables.ini, which asks for that URL, so the probing
# dominates the run. Override this when the credentials really do come from
# instance metadata.
